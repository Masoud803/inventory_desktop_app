...
// import AddCircleOutlineIcon from '@mui/icons-material/AddCircleOutline'; // Not used in provided code
...
const ProductFormModal = ({ 
    open, 
    onClose, 
    onSave, // Main product save callback
    productData, 
    allWebsites = [], 
    allCategories = [], 
    allSuppliers = [],
    mode // NEW PROP: 'editDetails', 'manageVariations', 'manageAccessories', or undefined for new product
}) => {
    const { token } = useAuth();
    // Product States
    ...
    const currentMode = isEditing ? mode : 'editDetails'; // New products always open in full 'editDetails' mode
    ...
    const resetFormStates = useCallback(() => {
        ...
        if (isEditing && productData && productData.id) {
            if (currentMode === 'manageVariations' && initialProductType === 'variable') {
                fetchVariations(productData.id);
            } else {
                setVariations([]);
            }
            if (currentMode === 'manageAccessories' && initialProductType === 'customisable') {
        ...
        setError('');
    }, [isEditing, productData, fetchVariations, fetchAccessories, currentMode]);
    ...
    const handleMainProductSubmit = async (event) => {
        ...
        // Ensure current_stock is only sent for simple products if it's being created or product details are edited
        let stockToSubmit = (isEditing && productData?.current_stock !== undefined) ? productData.current_stock : 0;
        if (productType === 'simple' && (currentMode === 'editDetails' || !isEditing)) {
             stockToSubmit = currentStock ? parseInt(currentStock) : 0;
        }
        ...
        try {
            ...
            onSave(); 
            if (!isEditing || currentMode === 'editDetails') { 
                handleClose(); 
            }
        } catch (err) {
    ...
    // --- Variation Management Functions (No changes from your original code) ---
    ...
    // --- Accessory Management Functions (No changes from your original code) ---
    ...
    const getDialogTitle = () => {
        if (!isEditing || !productData) return 'Add New Product';
        if (currentMode === 'editDetails') return `Edit Product: ${productData.name}`;
        if (currentMode === 'manageVariations') return `Manage Variations for: ${productData.name}`;
        if (currentMode === 'manageAccessories') return `Manage Accessories for: ${productData.name}`;
    ...
    return (
        <Dialog 
            open={open} 
            onClose={handleClose} 
            PaperProps={ (currentMode === 'editDetails' || !isEditing) ? { component: 'form', onSubmit: handleMainProductSubmit } : {} }
            maxWidth="md" 
            fullWidth
        >
            ...
            <DialogContent>
                ...
                {(currentMode === 'editDetails' || !isEditing) && (
                    <>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} sm={4}>
                                <FormControl fullWidth margin="dense" required 
                                    disabled={mainFormLoading || (isEditing && currentMode !== 'editDetails')}
                                >
                                    <InputLabel>Product Type</InputLabel>
                                    <Select value={productType} label="Product Type" 
                                        onChange={(e) => { 
                                            const newType = e.target.value;
                                            setProductType(newType); 
                                            // When creating new, or editing details, changing type might reset sub-items
                                            if (!isEditing || currentMode === 'editDetails') {
                                                if (newType !== 'variable') setVariations([]);
                                                if (newType !== 'customisable') setAccessories([]);
                                                // If changing to simple, also ensure stock field is available if it wasn't
                                            }
                                        }}
                                    >
                            ...
                            {(productType === 'simple' && (currentMode === 'editDetails' || !isEditing)) && (
                                <Grid item xs={12} sm={4}>
                                    <TextField margin="dense" id="currentStock" label="Current Stock" type="number" fullWidth value={currentStock} onChange={(e) => setCurrentStock(e.target.value)} disabled={mainFormLoading} />
                                </Grid>
                            )}
                            <Grid item xs={12} sm={productType === 'simple' && (currentMode === 'editDetails' || !isEditing) ? 4 : (currentMode === 'editDetails' || !isEditing ? 8 : 4 /* Adjust grid if stock is not shown */ )}> 
                                {/* This grid item might need better logic for span if stock is hidden */}
                                <TextField margin="dense" id="costOfGoods" label="Cost of Goods (Rs.)" type="number" fullWidth value={costOfGoods} onChange={(e) => setCostOfGoods(e.target.value)} disabled={mainFormLoading} InputProps={{ inputProps: { step: "0.01" } }} />
                            </Grid>
                ...
                {isEditing && productType === 'variable' && productData?.id && currentMode === 'manageVariations' && (
                    <Box component={Paper} sx={{ p: 2, mt: (currentMode !== 'editDetails' && !isEditing ? 0 : 3) }} elevation={2}>
                        <Typography variant="h6" gutterBottom>Manage Variations</Typography>
                         {/* Variation Form and List as in your original code */}
                ...
                {isEditing && productType === 'customisable' && productData?.id && currentMode === 'manageAccessories' && (
                     <Box component={Paper} sx={{ p: 2, mt: (currentMode !== 'editDetails' && !isEditing ? 0 : 3) }} elevation={2}>
                        <Typography variant="h6" gutterBottom>Manage Accessories</Typography>
                        {/* Accessory Form and List as in your original code */}
                        <Box sx={{display: 'flex', gap: 1, alignItems: 'flex-start', flexWrap:'wrap', mb:2}}>
            ...
            <DialogActions sx={{p: '16px 24px'}}>
                ...
                {(currentMode === 'editDetails' || !isEditing) && ( // Main save button only for product details or new product
                    <Button 
                        onClick={handleMainProductSubmit} 
                        type="submit" // It's part of PaperProps form, so it will submit
